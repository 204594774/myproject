...
presets = [
                ...
                "groups": [
                    ...
                    },
                    {
                        ...
                        "fields": [
                            ...
                            {"key": "advisor_name", "label": "指导教师", "type": "table", "required": True, "system": True}
                        ]
                    },
                    {
                        "title": "附件材料",
    ...
    },
    {
        ...
        "value": "math_modeling", # reusing for both math and electronic
        "data": {
            "title": "全国大学生数学建模竞赛",
            "system_type": "创新体系",
...
# Replace presetTemplates array
start_idx = content.find('presetTemplates: [')
if start_idx != -1:
    # find the matching closing bracket for presetTemplates
    bracket_count = 0
    end_idx = -1
    ...
    if end_idx != -1:
        new_presets_json = json.dumps(presets, indent=16, ensure_ascii=False)
        # fix indentation
        new_presets_json = new_presets_json.replace('\n', '\n            ')
        
        new_content = content[:start_idx] + 'presetTemplates: ' + new_presets_json + content[end_idx+1:]
        
        with open('static/js/app.js', 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("Updated presetTemplates successfully.")
    else:
        print("Could not find end of presetTemplates")
else:
    print("Could not find presetTemplates")
